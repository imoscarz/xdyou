...
class ClassTableActionMenu extends StatelessWidget {
  ...
  final String iCalendarText;
  final Future<void> Function(ClassDetail classDetail, TimeArrangement time)
  onAddClass;
  final Future<bool> Function(Future<void> Function() showPermissionDialog)
  onOutputToCalendar;
  final Future<void> Function(BuildContext context) onRefreshClassTable;
  ...
  const ClassTableActionMenu({
    ...
    required this.iCalendarText,
    required this.onAddClass,
    required this.onOutputToCalendar,
    required this.onRefreshClassTable,
  ...
  Future<void> _openNotArrangedClasses(BuildContext context) async {
    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => NotArrangedClassList(notArranged: notArranged),
      ),
    );
  }
  ...
  Future<void> _openClassChanges(BuildContext context) async {
    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (context) => ClassChangeList(classChanges: classChanges),
      ),
    );
  }
  ...
  Future<void> _addClass(BuildContext context) async {
    ...
    if (context.mounted && data != null) {
      await onAddClass(data.$1, data.$2);
    }
  }
  ...
  Future<void> _exportICalendar(BuildContext context) async {
    try {
      ...
      await FilePicker.saveFile(
        ...
        bytes: Uint8List.fromList(iCalendarText.codeUnits),
  ...
  Future<void> _outputToSystemCalendar(BuildContext context) async {
    final didOutput = await onOutputToCalendar(() async {
      await showDialog<void>(
        context: context,
        builder: (context) => AlertDialog(
          title: Text(
  ...
  Future<void> _refreshClassTable(BuildContext context) async {
    ...
    if (context.mounted && isAccepted) {
      await onRefreshClassTable(context);
      if (context.mounted) {
        showToast(
          context: context,
          msg: FlutterI18n.translate(
